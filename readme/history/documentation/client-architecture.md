...
- **Native Resume discovery**: `e-tui::resume` and the Resume Input Page own viewport-sized demand, progressive search, selection, and page/workspace result admission. The Pi runner services that demand through one background blocking index job at a time, admitting each batch to a frame before requesting another; directory enumeration and bounded reverse-title/first-user reads never run under UI locks or inside the RPC request reducer. Session files remain read-only, and oversized or partly damaged post-header metadata degrades only the title. Adapters supply modification timestamps, and the frontend formats relative ages; only visible rows contribute age-change deadlines to the shared presentation scheduler, without filesystem access or transcript-cache invalidation. DSH retains its existing progressive list transport.
...
- **Context injection text**: generic prompt-injection events (`CardRole::Context`) render as plain text, not a
  ...
  failed reads retain visible failure feedback, and explicit user skill cards remain unchanged.
...
- **Overlays and Input Page rendering**: a command prompt that truly draws over the transcript must first call
  ...
  page-internal dividers are Umber, focused text is Sage, and selected text is Coral, except Resume's fixed-tone title/age columns with a separate focus marker. Settings keeps its category
