...
2. `[semantics.*]` is the fixed semantic schema, containing `surface`, `markdown`, `markdown_weak`, `code`, `code_weak`, `input` (including the status
   bar), `working_status`, `log`, `activity`, `card`, `overlay`, `diff`, `separator`. The `code`/`code_weak` groups carry the dedicated code-coloring roles (`text`, `comment`, `keyword`, `type`, `function`, `string`, `constant`, `attribute`, `escape`, `invalid`, `meta`); the block fill background stays under `markdown.code_block_bg`. Each fixed role is a style object with only `fg`
   ...
   text, its `bg` the box fill). Separator backgrounds come from these semantic roles, with an omitted value
   inheriting the themed base surface; render code must not substitute a hard-coded palette color.
