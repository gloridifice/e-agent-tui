//! Project-local execution-history persistence owned by the Pi adapter.
...
const IGNORE_ENTRY: &str = "/e-pi/execution-history/";
...
mod tests {
    ...
    fn recorder_waits_for_attachment_and_switches_exclusive_session_writers() {
        let root = tempfile::tempdir().unwrap();
        let mut recorder = HistoryRecorder::new("e-pi");
