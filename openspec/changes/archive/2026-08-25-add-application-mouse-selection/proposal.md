...
- Normalize raw Backspace bytes on Windows so local Windows Terminal maps `0x7f` to Backspace and `0x08` to Ctrl+Backspace without depending on asynchronously sampled key state.
...
- Windows Terminal input compatibility: raw Backspace normalization and parser regression tests; no bridge or wire-protocol change.
