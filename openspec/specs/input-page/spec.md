...
The Input Page refactor SHALL use the existing login, proxy, model, and configuration messages and SHALL NOT require a WebSocket protocol-version or Node bridge change.

#### Scenario: Send page action
- **WHEN** an Input Page performs a login, proxy, or model operation
- **THEN** the client emits the existing corresponding `ClientMessage` shape without introducing a new wire message type

### Requirement: Non-configuration surfaces remain independent
The session picker, help overlay, copy mode, approval interaction, and question bar SHALL remain outside the Input Page roster in this change.
