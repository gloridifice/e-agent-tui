# Operational guides

Current procedures for building, deploying, validating, and diagnosing the project. Requirements and architecture live under [doco/](../doco/README.md).

- [Development](development.md) — checkout install, build, release, and diagnostic entry points.
- [Testing and validation](testing.md) — scoped tests, protocol checks, and compatibility gates.
- [DSH integration](dsh-integration.md) — profile setup, bridge deployment, restart, and upgrades.
- [Troubleshooting](troubleshooting.md) — operational symptoms and diagnostic routes.
- [Key mappings](key-mapping.md) — configuration, reload, and terminal limitations.
- [Terminal binding gate](terminal-binding-gate.md) — keyboard compatibility checks.
- [Performance measurement](performance.md) — repeatable profiling and frame measurement.

Historical audits, migration records, superseded documentation, and archived designs live under [history](history/README.md) and [archive](archive/README.md). They are context only.
