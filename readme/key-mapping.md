...
- `/reload` refreshes the mapping and subsequent help/hints. Saving ordinary settings never rewrites `key_mapping.toml`. The help shortcut and `/help` open the same centered Markdown modal; reopening it returns to the top and uses the latest effective mapping. Help does not append a transcript message or enumerate slash-command catalogs.
...
Full-screen browsing combines `full_screen` navigation with its page-specific scope. Execution history shows only the session-wide Top 50 ranking; its `history` context inherits row, half-page, full-page, and exit actions from `full_screen`. The centered Help modal inherits row, half-page, and full-page movement from `full_screen`, while `[help].close` remains its own binding; the mouse wheel also scrolls Help. Disabled actions do not fall through to composer or transcript behavior. There is no history view-toggle binding; the retired `history.toggle_view` override is ignored for compatibility and can be removed from existing files. Fixed footers are generated from the effective bindings after overrides. Mouse-wheel movement and visible-screen text selection remain available independently of keyboard mappings.
