...
Full-screen browsing combines `full_screen` navigation with its page-specific scope. Execution history opens on the session-wide Top 50 ranking and switches to the chronological usage timeline through `[history].toggle_view` (Tab); its `history` context inherits row, half-page, full-page, and exit actions from `full_screen`, and each view keeps its own scroll offset. The centered Help modal inherits row, half-page, and full-page movement from `full_screen`, while `[help].close` remains its own binding; the mouse wheel also scrolls Help. Disabled actions do not fall through to composer or transcript behavior. Fixed footers are generated from the effective bindings after overrides. Mouse-wheel movement and visible-screen text selection remain available independently of keyboard mappings.
