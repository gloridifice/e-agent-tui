...
#### Scenario: 同步当前版本
- **WHEN** canonical contract 的 `protocolVersion` 为 4
- **THEN** Rust `WIRE_PROTOCOL_VERSION`、bridge `PROTOCOL_VERSION`、`bridge/package.json` 的 `wireProtocol` 和生成文档全部为 4
...
Contract MUST 为所有 client messages 和所有非 HostEvent server frames 描述 required/optional 字段、wire 字段名和基本容器/值类型；Rust 与 Node 测试 MUST 验证各自序列化、解析或 shaping 与这些描述一致。

#### Scenario: Client message 字段一致
- **WHEN** 对 `login-proxy-create`、`model-set`、`history` 或其他 client message 生成 conformance sample
