...
- Clearing a nonempty text-only composer with `clear_or_quit` MUST append its exact text to the existing bounded prompt history so the next history recall retrieves it, without submitting it. Image-bearing drafts remain excluded from prompt history; empty-input behavior is unchanged.
...
- Slash-command and declared argument completion MUST match the editable prefix before the cursor at the start of the composer or immediately after a model-mark prefix, even when a draft follows it. Completion after a model mark MUST NOT change marked-prompt submission semantics. Candidate navigation and acceptance MUST replace only the command prefix, preserving the model mark, trailing text, and atomic paste/image blocks; a missing separator is inserted without rewriting the suffix. Esc MUST restore the pre-selection prefix and cursor. Moving into the preserved suffix or beyond the first line MUST close command completion. With a preserved suffix, Enter MUST fill and dismiss without submitting; a subsequent Enter submits. Without a suffix, existing direct acceptance remains unchanged.
